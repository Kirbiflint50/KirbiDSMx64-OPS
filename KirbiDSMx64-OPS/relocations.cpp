...
Relocations::Relocations(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Relocations)
{
    ui->setupUi(this);
}
...
void Relocations::on_Relocations_windowTitleChanged(const QString &title)
{
    QPalette palete;
   ...
   std::string rd = datread.str();
    QString dts = QString::fromStdString(rd);
        ...
        std::ifstream pe_file(gtrlpth.toStdString(), std::ios::in | std::ios::binary);
        if(!pe_file)
        {
        ...
        ui->plainTextEdit->appendPlainText(QString::fromStdString(relocat.str()));
}
